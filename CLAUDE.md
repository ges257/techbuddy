...
- Backend: Claude API (Opus 4.6) with extended thinking + prompt caching, 25 tools via direct tool-use loop (up to 5 rounds)
- Dispatch layer in `mcp_servers/screen_dispatch.py`: 25 tools with tiered fallback: win32com (Tier 1) → pywinauto (Tier 2) → existing MCP (Tier 3) → Claude Vision (Tier 4, read_my_screen captures screenshots)
...
- Test: `cd ~/techbuddy && venv/bin/pytest tests/ -v` (117/117 passing)
- Lint: `ruff check .`
